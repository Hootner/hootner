...
class Config:
    ...
    def _validate_model(self):
        ...
        if model.get("size") not in valid_sizes:
            raise ConfigurationError(f"model.size must be one of: {valid_sizes}")

        valid_devices = ["cpu", "cuda", "mps"]
        if model.get("device") not in valid_devices:
            raise ConfigurationError(f"model.device must be one of: {valid_devices}")
    ...
    def _validate_generation(self):
        ...
        if gen["default_width"] > gen["max_width"]:
            raise ConfigurationError("generation.default_width cannot exceed max_width")
...
if __name__ == "__main__":
    ...
    print(
        f"  Resolution: {config.get('generation.default_height')}x{config.get('generation.default_width')}"
    )
