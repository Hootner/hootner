...
class GaussianDiffusion:
    ...
    def ddim_sample(
        ...
        for i, t in enumerate(timesteps):
            ...
            # Predict x_0
            pred_x0 = (video - torch.sqrt(1 - alpha_t) * pred_noise) / torch.sqrt(alpha_t)
    ...
    def progressive_sampling(
        ...
        for stage in range(num_stages):
            print(f"🎬 Stage {stage + 1}/{num_stages} - Resolution: {current_shape[-2:]} ")
            
            # Generate at current resolution
            video = self.ddim_sample(model, current_shape, context, steps=25, device=device)
...
class ClassifierFreeGuidance:
    ...
    def guided_sample(
        ...
        timesteps = list(range(0, diffusion.timesteps, diffusion.timesteps // steps))[::-1]
        ...
        for t in tqdm(timesteps, desc="Guided Sampling"):
            ...
            if method == "ddim":
                # DDIM update
                alpha_t = diffusion._extract(diffusion.alphas_cumprod, t_batch, video.shape)
