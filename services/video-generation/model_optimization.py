...
class ModelOptimizer:
    ...
    @staticmethod
    def compile_model(
        model: nn.Module, mode: str = "default", fullgraph: bool = False
    ) -> nn.Module:
        """
    ...
    @staticmethod
    def profile_model(
        model: nn.Module, input_shape: tuple, device: str = "cuda"
    ) -> dict:
        """
...
class EMAModel:
    ...
    def __init__(
        self, model: nn.Module, decay: float = 0.9999, device: Optional[str] = None
    ):
        self.model = model
        self.decay = decay
    ...
    def state_dict(self) -> dict:
        """Get EMA state dict"""
        return {"decay": self.decay, "ema_params": self.ema_params}
...
def export_to_onnx(
    model: nn.Module, output_path: str, input_shape: tuple, opset_version: int = 14
):
    """
...
def benchmark_model(
    model: nn.Module, input_shapes: list, device: str = "cuda", iterations: int = 100
) -> dict:
    """
