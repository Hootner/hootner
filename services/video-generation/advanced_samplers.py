...
class DPMSolverPlusPlus:
        ...
        for i in tqdm(range(num_steps), desc="DPM-Solver++ sampling"):
            ...
            if self.solver_order == 1 or len(model_outputs) < 2:
                ...
                x = self._second_order_update(
                    x, model_outputs[-2], model_output, lambda_t, lambdas[i-1], lambda_next
                )
            else:
                # Third-order update
                x = self._third_order_update(
                    x, model_outputs[-3], model_outputs[-2], model_output,
                    lambda_t, lambdas[i-2], lambdas[i-1], lambda_next
                )
    ...
    def _first_order_update(
        ...
        x_next = (
            ...
            - sigma_next * torch.expm1(h) * model_output
        )
    ...
    def _third_order_update(
        ...
        D2 = ((1 + 1 / (2 * r_1)) / r_0) * (model_output_2 - model_output_1) - (1 / (2 * r_0 * r_1)) * (model_output_1 - model_output_0)
...
class EulerAncestralSampler:
    ...
    def sample(
        ...
        for i in tqdm(range(num_steps), desc="Euler Ancestral sampling"):
            t = timesteps[i]
            sigma = sigmas[i]
            sigma_next = sigmas[i + 1] if i < num_steps - 1 else torch.tensor(0.0, device=device)
