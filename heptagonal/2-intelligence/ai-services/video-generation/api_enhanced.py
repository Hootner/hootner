...
class JobManager:
    ...
    def _execute_job(self, job_data: dict):
        ...
        try:
            ...
            socketio.emit(
                "error", {"job_id": job_id, "error": str(e)}, namespace="/generation"
            )
        ...
        if self.redis_client:
            try:
                self.redis_client.setex(
                    f"video:{cache_key}", CACHE_TTL, json.dumps(data)
                )
                return
            except:
                pass
    ...
    return jsonify(
        {
            ...
            "active_jobs": len(
                [j for j in job_manager.jobs.values() if j["status"] == "processing"]
            ),
...
def get_metrics():
    ...
    output = []
    output.append(
        f"# HELP video_gen_requests_total Total number of generation requests"
    )
