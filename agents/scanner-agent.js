...
class ScannerAgent {
  ...
  async scanAPI() {
    ...
    try {
      ...
      if (!response.ok) issues.push({ type: 'api', message: 'API unhealthy', priority: 'high' });
    } catch (e) {
  ...
  async saveIssues(issues) {
    const data = issues.map(i => ({ ...i, id: Date.now() + Math.random(), status: 'pending', timestamp: new Date().toISOString() }));
