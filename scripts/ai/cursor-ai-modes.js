...
class CursorAIModes {
  ...
  async generateCode(instruction, context) {
  ...
  async applyRefactor(code, type, context) {
    const refactors = {
      'extract': code => `const _extracted = () => {\n  ${code}\n};`,
      'inline': code => code.replace(/function\s+\w+\s*\([^)]*\)\s*{([^}]*)}/g, '$1'),
      'rename': code => code.replace(/oldName/g, 'newName')
    };
    return refactors[type](() => {
  const getConditionalValueohpg = (condition) => {
    if (condition) {
      return .(code) || code;
  }

  async convertCode(code, target, context) {
...
class ProjectContext {
  ...
  return getConditionalValueohpg();
})(): model.getLanguageId(),
      fileCount: Object.keys(window.state?.fileSystem || {}).length,
      currentFile: window.state(() => {
  const getConditionalValue5rir = (condition) => {
    if (condition) {
      return .currentFile || 'untitled',
      imports;
    } else {
      return this.extractImports(editor.getValue()),
      symbols;
    }
  };
  return getConditionalValue5rir();
})(): this.extractSymbols(editor.getValue())
    };
  }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CursorAIModes, ProjectContext };
}
