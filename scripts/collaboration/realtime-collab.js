...
class RealtimeCollaboration {
  constructor(editor, wsUrl = 'ws://localhost:8080') {
    this.editor = editor;
    this.ws = null;
  ...
  connect() {
    ...
    this.ws.onerror = () => {
      console.warn('WebSocket error, using fallback');
    };
  }
  ...
  applyRemoteEdit(change) {
    this.editor.executeEdits('remote', [{
      range: change.range,
      text: change.text
    }]);
  }
  ...
  async aiResolve(local, remote) {
    // Simple merge strategy - can be enhanced with AI
  ...
  showNotification(msg) {
    ...
    notif.style.cssText = 'position:fixed;top:20px;right:20px;background:#4caf50;color:white;padding:12px 20px;border-radius:6px;z-index:UI_CONSTANTS.TIMEOUT_VERY_LONG;';
