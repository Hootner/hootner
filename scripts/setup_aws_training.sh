...
sudo apt-get install -y curl wget git awscli
...
S3_BUCKET=${HOOTNER_TRAINING_S3_BUCKET:-hootner-uploads-504165876439}
S3_SCRIPTS_PREFIX=${HOOTNER_TRAINING_S3_SCRIPTS_PREFIX:-training-scripts/}

# Download training scripts from S3 (optional; do not fail setup if missing)
echo "⬇️  Attempting to download training scripts from s3://${S3_BUCKET}/${S3_SCRIPTS_PREFIX}"
aws s3 cp "s3://${S3_BUCKET}/${S3_SCRIPTS_PREFIX}" . --recursive || \
    echo "⚠️  Could not download training scripts from S3 (continuing)."
