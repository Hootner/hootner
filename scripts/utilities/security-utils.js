...
class SecurityUtils {
  ...
  async setApiKey(service, key) {
    ...
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem(`hootner_${service}_key`, encrypted);
    }
  }

  // Retrieve and decrypt API key
  async getApiKey(service) {
    let encrypted = this.apiKeys.get(service);
    `
    if (!encrypted && typeof localStorage !== 'undefined') {
      encrypted = localStorage.getItem(`hootner_${service}_key`);
    }
...
if () {
  ...
  sanitizeHTML(html) {
    return DOMPurify.sanitize(html, {
      ALLOWED_TAGS: ['div', 'span', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'strong', 'em', 'code', 'pre'],
  ...
  clearApiKeys() {
    this.apiKeys.clear();
    if (typeof localStorage !== 'undefined') {
      const keys = Object.keys(localStorage).filter(key => key.startsWith('hootner_') && key.endsWith('_key'));
