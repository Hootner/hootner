...
if (process.env.JWT_SECRET.length < 32 || process.env.SESSION_SECRET.length < 32) {
  logger.error('Secrets must be at least 32 characters');
  process.exit(1);
}
...
app.use(
  compression({
    level: config.server.compressionLevel,
...
const validateFile = (file) => {
  const allowedTypes = /\.(mp4|webm|ogg|avi|mov)$/i;
...
app.use(
  ...
  express.static(uploadsDir, {
    maxAge: config.server.uploadsMaxAge,
    etag: true,
    lastModified: true,
    immutable: true,
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        logger.error('File upload verification failed', { ip: req.ip });
        return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Upload verification failed' });
      }
      ...
      logger.error('Upload failed', { error: error.message, ip: req.ip });
      if (filePath) {
        try {
          if (fs.existsSync(filePath)) {
...
app.get('/api/videos', async (req, res) => {
  try {
    ...
    logger.error('Failed to read videos', { error: error.message });
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to retrieve videos' });
  }
});

app.get('/api/download', authenticateJWT, validate({ url: schemas.string(1, 200) }), async (req, res) => {
  const { url } = req.query;

  if (typeof url !== 'string' || /%2e|%2f/i.test(url)) {
    logger.warn('Invalid URL encoding detected', { url, ip: req.ip });
    return res.status(HTTP_STATUS.BAD_REQUEST).json({ error: 'Invalid URL' });
  }

  const decodedUrl = decodeURIComponent(url);
  const filename = path.basename(decodedUrl).replace(/[^a-zA-Z0-9._-]/g, '_');
  if (!filename || filename.includes('..') || filename.startsWith('.') || filename.length > LIMITS.MAX_STRING_LENGTH) {
    logger.warn('Invalid download attempt', { url, ip: req.ip });
  ...
  if (!normalizedFilePath.startsWith(normalizedUploadsDir + path.sep)) {
    logger.warn('Path traversal attempt blocked', { filename, ip: req.ip });
  ...
  try {
    ...
    if (err) {
      logger.warn('File access error', { error: err.message, filename, ip: req.ip });
    }
...
// Serve HOOTNER app/
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'electron-code-editor/index.html'));
});
...
const server = app
  .listen(PORT, '::', () => {
    logger.info('HOOTNER Server started', { port: PORT, ipv4: `http://localhost:${PORT}`, ipv6: `http://[::1]:${PORT}` });
  })
  .on('error', (err) => {
    if (err) {
      logger.error('Failed to start HOOTNER server', { error: err });
      process.exit(1);
    }
  });

setupGracefulShutdown(server, 'HOOTNER', {
  cleanup: [() => memoryMonitor.cleanup()],
});