...
const commands = { dev: `docker-compose -f docker-compose.dev.yml up -d ${service !== 'all' ? service : ''}`,
...
try { execSync(commands[env] || commands.dev, { stdio: 'inherit' }); } catch (error) { console.error('❌ Deployment failed: ', error.message);
  process.exit(1);' }
