...
```javascript
/* BROKEN: */
item.innerHTML = `<p><strong>${activity.text}</strong></p>`

/* FIX: */
const sanitizer = window.Sanitizer || { sanitize: (html) => DOMPurify.sanitize(html) }
item.innerHTML = sanitizer.sanitize(`<p><strong>${activity.text}</strong></p>`)
```
...
/* FIX: Add timeout or use simpler validation */
