...
const parseIntSafe = (value, defaultValue, min = 0, max = Number.MAX_SAFE_INTEGER) => {
...
const config = {
  // Audit Service
  audit: {
    port: parseIntSafe(process.env.AUDIT_SERVICE_PORT, LIMITS.MIN_PORT, 1024, LIMITS.MAX_PORT),
  ...
  // Backup Service
  backup: {
    port: parseIntSafe(process.env.BACKUP_SERVICE_PORT, LIMITS.HUB_PORT, 1024, LIMITS.MAX_PORT),
    ...
    rateLimit: {
      windowMs: parseIntSafe(process.env.BACKUP_RATE_WINDOW, TIMEOUTS.ONE_MINUTE, TIMEOUTS.ONE_SECOND, TIMEOUTS.ONE_HOUR),
  ...
  // Security Service
  security: {
    port: parseIntSafe(process.env.SECURITY_SERVICE_PORT, LIMITS.DEFAULT_PORT, 1024, LIMITS.MAX_PORT),
  ...
  rateLimit: {
    default: {
      windowMs: parseIntSafe(process.env.RATE_LIMIT_WINDOW, TIMEOUTS.ONE_MINUTE, TIMEOUTS.ONE_SECOND, TIMEOUTS.ONE_HOUR),
      max: parseIntSafe(process.env.RATE_LIMIT_MAX, 100, 1, LIMITS.MAX_RETRY_COUNT),
    },
    auth: {
      windowMs: parseIntSafe(process.env.AUTH_RATE_WINDOW, TIMEOUTS.ONE_MINUTE, TIMEOUTS.ONE_SECOND, TIMEOUTS.ONE_HOUR),
      max: parseIntSafe(process.env.AUTH_RATE_MAX, 20, 1, 1000),
    },
    payment: {
      windowMs: parseIntSafe(process.env.PAYMENT_RATE_WINDOW, TIMEOUTS.ONE_MINUTE, TIMEOUTS.ONE_SECOND, TIMEOUTS.ONE_HOUR),
      max: parseIntSafe(process.env.PAYMENT_RATE_MAX, 10, 1, 1000),
    },
    security: {
      windowMs: parseIntSafe(process.env.SECURITY_RATE_WINDOW, TIMEOUTS.ONE_MINUTE, TIMEOUTS.ONE_SECOND, TIMEOUTS.ONE_HOUR),
      max: parseIntSafe(process.env.SECURITY_RATE_MAX, 50, 1, 1000),
    },
    audit: {
      windowMs: parseIntSafe(process.env.AUDIT_RATE_WINDOW, TIMEOUTS.ONE_MINUTE, TIMEOUTS.ONE_SECOND, TIMEOUTS.ONE_HOUR),
      max: parseIntSafe(process.env.AUDIT_RATE_MAX, LIMITS.RATE_LIMIT_MIN, 1, LIMITS.MAX_RETRY_COUNT),
    },
    analytics: {
      windowMs: parseIntSafe(process.env.ANALYTICS_RATE_WINDOW, TIMEOUTS.ONE_MINUTE, TIMEOUTS.ONE_SECOND, TIMEOUTS.ONE_HOUR),
  ...
  // Circuit Breaker
  circuitBreaker: {
    threshold: parseIntSafe(process.env.CIRCUIT_BREAKER_THRESHOLD, 5, 1, 100),
    timeout: parseIntSafe(process.env.CIRCUIT_BREAKER_TIMEOUT, TIMEOUTS.ONE_MINUTE, TIMEOUTS.ONE_SECOND, TIMEOUTS.FIVE_MINUTES),
  },

  // Retry Logic
  retry: {
    maxRetries: parseIntSafe(process.env.MAX_RETRIES, 3, 0, 10),
  ...
  // Timeout
  timeout: {
    default: parseIntSafe(process.env.DEFAULT_TIMEOUT, TIMEOUTS.FIVE_SECONDS, TIMEOUTS.ONE_SECOND, TIMEOUTS.FIVE_MINUTES),
  ...
  // Server
  server: {
    port: parseIntSafe(process.env.PORT, TIMEOUTS.FIVE_SECONDS, 1024, LIMITS.MAX_PORT),
