...
function validateGenerationInput(input) {
    ...
    if (!input.prompt || input.prompt.length < 5) {
        errors.push({ field: 'prompt', message: 'Prompt must be at least 5 characters' });
    }
    ...
    if (input.numFrames && (input.numFrames < 4 || input.numFrames > 64)) {
        errors.push({ field: 'numFrames', message: 'Frames must be between 4 and 64' });
    }

    if (input.height && (input.height < 32 || input.height > 512)) {
        errors.push({ field: 'height', message: 'Height must be between 32 and 512' });
    }

    if (input.width && (input.width < 32 || input.width > 512)) {
        errors.push({ field: 'width', message: 'Width must be between 32 and 512' });
    }
