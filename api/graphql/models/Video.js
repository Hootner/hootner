...
async function createVideo({ title, description, url, thumbnailUrl, userId, resolution, format, size, metadata }) {
...
async function getVideoById(id) {
  const res = await docClient.send(new GetCommand({ TableName: TABLE_NAME, Key: videoKey(id) }));
...
async function listVideos(filter = {}, limit = 20, offset = 0) {
  ...
  if (filter.search) {
    ...
    items = items.filter(v => (v.title || '').toLowerCase().includes(term) || (v.description || '').toLowerCase().includes(term));
  }
...
async function trendingVideos(limit = 10) {
  ...
  const items = (res.Items || []).sort((a, b) => (b.views || 0) - (a.views || 0)).slice(0, limit);
...
async function incrementViews(id) {
  await docClient.send(new UpdateCommand({
    TableName: TABLE_NAME,
    Key: videoKey(id),
    UpdateExpression: 'SET views = if_not_exists(views, :zero) + :one, updatedAt = :now',
...
async function updateVideo(id, input) {
  ...
  await docClient.send(new PutCommand({ TableName: TABLE_NAME, Item: { ...videoKey(id), ...next, entityType: 'VIDEO' } }));
...
async function toggleLike(videoId, userId) {
  ...
  if (idx > -1) likes.splice(idx, 1); else likes.push(userId);
  video.likes = likes;
  video.updatedAt = new Date().toISOString();
  await docClient.send(new PutCommand({ TableName: TABLE_NAME, Item: { ...videoKey(videoId), ...video, entityType: 'VIDEO' } }));
...
export { createVideo, getVideoById, listVideos, trendingVideos, incrementViews, updateVideo, deleteVideo, toggleLike, addComment, deleteComment };
