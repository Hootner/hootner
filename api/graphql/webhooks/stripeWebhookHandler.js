...
class StripeWebhookHandler {
    ...
    async handleSubscriptionUpdated(subscription, event) {
        const { customer, id, status, cancel_at_period_end, items, current_period_end } = subscription;
    ...
    async handleTrialWillEnd(subscription, event) {
        ...
        try {
            ...
            await this.sendSubscriptionEmail(customer, 'trial_ending', {
                ...
                daysRemaining: Math.ceil((trial_end * 1000 - Date.now()) / (1000 * 60 * 60 * 24)),
            });
    ...
    async handleInvoicePaymentFailed(invoice, event) {
        ...
        logger.error('Invoice payment failed', {
            ...
            nextAttempt: next_payment_attempt ? new Date(next_payment_attempt * 1000).toISOString() : null,
        });
