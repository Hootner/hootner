...
function validateEnvVar(name, options = {}) {
    ...
    if (minLength > 0 && value.length < minLength) {
        throw new Error(`Environment variable ${name} must be at least ${minLength} characters (current: ${value.length})`);
    }
    ...
    try {
        ...
        try {
            config.JWT_SECRET = validateEnvVar('JWT_SECRET', {
                required: true,
                minLength: 32,
                defaultValue: config.NODE_ENV === 'production' ? null : 'dev-jwt-secret-minimum-32-characters-long'
            });
        } catch (err) {
        ...
        try {
            config.SESSION_SECRET = validateEnvVar('SESSION_SECRET', {
                required: true,
                minLength: 32,
                defaultValue: config.NODE_ENV === 'production' ? null : 'dev-session-secret-minimum-32-chars'
            });
        } catch (err) {
            errors.push(err.message);
        }
