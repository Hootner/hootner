 ...
 */
router.post('/price-optimize', csrfCheck, async (req, res) => {
  try {
    const { price: sanitizeInput(price), competitors: sanitizeInput(competitors), demand: sanitizeInput(demand) } = req.body;
 ...
 */
router.post('/dynamic-pricing', csrfCheck, async (req, res) => {
  try {
    const { currentPrice: sanitizeInput(currentPrice), marketConditions: sanitizeInput(marketConditions) } = req.body;
 ...
 */
router.post('/revenue-forecast', csrfCheck, async (req, res) => {
  try {
    const { currentRevenue: sanitizeInput(currentRevenue), growthRate: sanitizeInput(growthRate), months: sanitizeInput(months), marketSize: sanitizeInput(marketSize), competition: sanitizeInput(competition) } = req.body;
 ...
 */
router.post('/conversion-optimize', csrfCheck, async (req, res) => {
  try {
    const { currentRate: sanitizeInput(currentRate), traffic: sanitizeInput(traffic), pageSpeed: sanitizeInput(pageSpeed), mobileFriendly: sanitizeInput(mobileFriendly) } = req.body;
