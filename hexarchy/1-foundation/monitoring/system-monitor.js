...
class SystemMonitor {
  ...
  async _getServiceMetrics() {
    ...
    const domains = ['foundation', 'intelligence', 'communication', 'interface', 'economy', 'governance', 'data', 'operations'];
  ...
  async _checkThresholds(metrics) {
    ...
    } else if (metrics.system.memory.usage > this.thresholds.get('memory_usage').warning) {
      alerts.push({
        type: 'memory_usage',
        severity: 'warning',
        value: metrics.system.memory.usage,
  ...
  async _processAlert(alert) {
    ...
    // Publish alert event
    const alertEvent = new DomainEvent(
   ...
   */
  getDashboardData(timeRange = 3600000) { // Last hour
  ...
  getStats() {
    return {
      ...
      lastCollection: this.metrics.has('system_snapshot') ? 
        Math.max(...this.metrics.get('system_snapshot').map(m => m.timestamp)) : null
    };
  }
}
