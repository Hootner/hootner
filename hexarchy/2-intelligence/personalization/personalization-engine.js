...
class PersonalizationEngine {
  ...
  async detectLearningStyle(userId, behaviorData) {
    const { videoWatchTime, textReadTime, interactiveTime, audioListenTime } = behaviorData;
    ...
    const detectedStyle = Object.entries(scores)
  ...
  async updateProfile(userId, sessionData) {
    ...
    // Update streak
    const daysSinceLastActive = (Date.now() - profile.history.lastActive) / (1000 * 60 * 60 * 24);
