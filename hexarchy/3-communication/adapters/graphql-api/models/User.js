...
async function listUsers(limit = 20, offset = 0) {
  ...
  const items = (res.Items || []).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
...
async function createUser(input) {
  ...
  const item = {
    ...
    name: input.name,
    avatar: input.avatar,
    subscription: SUBSCRIPTION_LEVELS.includes(input.subscription) ? input.subscription : 'FREE',
