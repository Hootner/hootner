...
async function getConversation(id) {
  const res = await docClient.send(new GetCommand({ TableName: TABLE_NAME, Key: conversationKey(id) }));
...
async function listConversations(userId, limit = 50) {
  ...
  return items.sort((a, b) => new Date(b.lastMessageAt) - new Date(a.lastMessageAt)).slice(0, limit);
}
...
async function updateLastMessage(conversationId, lastMessage) {
  ...
  await docClient.send(new UpdateCommand({
    ...
    UpdateExpression: 'SET lastMessage = :msg, lastMessageAt = :now, updatedAt = :now',
