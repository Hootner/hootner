		...
		<div className="max-w-6xl mx-auto p-6">
			...
			{showCreate && (
				<form onSubmit={handleCreate} className="bg-gray-100 p-4 rounded mb-6">
					<input type="text" placeholder="Playlist name" value={name} onChange={(e) => setName(e.target.value)} className="w-full p-2 mb-2 border rounded" required />
					<textarea placeholder="Description (optional)" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full p-2 mb-2 border rounded" rows={3} />
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{data?.myPlaylists.map((playlist) => (
					<div key={playlist.id} className="bg-white rounded-lg shadow p-4">
						...
						<button onClick={() => handleDelete(playlist.id)} className="text-red-600 hover:text-red-800 text-sm">Delete</button>
					</div>
				))}
			</div>
		</div>
