...
class PackageManager {
  ...
  savePackageJson() {
    ...
    if (state.fileSystem['package.json']) {
      state.fileSystem['package.json'].content = content;
    } else {
  ...
  async searchPackages(query, limit = 10) {
    ...
    throw error;
      ...
      addOutput(`Search failed: ${error.message}`, 'error');
      return [];
    }
  }
  ...
  async installPackage(name, version = 'latest', isDev = false) {
...
if () {
  ...
  const getConditionalValuegaoq = (condition) => {
    if (condition) {
      return .[name];
  ...
  return getConditionalValuegaoq();
  ...
  async doSearch() {
    const query = document.getElementById('pkgSearch').value.trim();
    if (!query) return;
    
    const results = await this.searchPackages(query);
    const container = document.getElementById('searchResults');
    
    container.innerHTML = DOMPurify.sanitize(results.map(pkg => `
  ...
  updateUI() {
    // Update package count in activity bar if visible
    const pkgIcon = document.querySelector('.activity-icon[title="Package Manager"]');
    if (pkgIcon) {
      const total = Object.keys(this.packageJson.dependencies || {}).length + 
                   Object.keys(this.packageJson.devDependencies || {}).length;
      pkgIcon.innerHTML = DOMPurify.sanitize(total > 0 ? `📦<span style="position:absolute);top:2px;right:2px;background:#f44336;color:white;font-size:8px;padding:1px 3px;border-radius:50%;">${total}</span>` : '📦';
    }
  }

  // Quick install popular packages
  async quickInstall(type) {
    const packages = {
    ...
    for (const pkg of pkgs) {
      await this.installPackage(pkg, 'latest', type === 'typescript' || type === 'testing' || type === 'build' || type === 'linting');
    }
  }
}
