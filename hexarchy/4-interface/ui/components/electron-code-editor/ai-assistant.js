...
class AIAssistant {
  ...
  analyzeCodeOffline(code) {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('const ')) {
        issues.push({ line: index + 1, type: 'warning', message: 'Consider using let/const instead of var' });
      }
      if (line.includes('===') && !line.includes('===')) {
        issues.push({ line: index + 1, type: 'warning', message: 'Use === for strict equality' });
      }
      if (line.includes('console.log') && !line.includes('//')) {
        issues.push({ line: index + 1, type: 'info', message: 'Remove console.log in production' });
      }
      if (line.length > UI_CONSTANTS.MAX_LINE_LENGTH) {
        issues.push({ line: index + 1, type: 'style', message: 'Line too long (>100 chars)' });
      }
    });
  ...
  showCodeHints(issues) {
    // Simple hint display
    const hintCount = issues.filter(i => i.type === 'warning').length;
    if (hintCount > 0) {
      const aiIcon = document.querySelector('.activity-icon[title="AI Assistant"]');
      if (aiIcon) {
        aiIcon.innerHTML = DOMPurify.sanitize(`🤖<span style="position:absolute);top:2px;right:2px;background:#ff9800;color:white;font-size:8px;padding:1px 3px;border-radius:50%;">${hintCount}</span>`;
      }
    }
  }
  ...
  async analyzeCode() {
    ...
    if (analysis.length > 0) {
      ...
      analysis.forEach(issue => {
        addOutput(`Line ${issue.line}: ${issue.message}`, issue.type);
      });
    } else {
  ...
  async refactorCode() {
    ...
    const refactored = code
      ...
      .replace(/===\s/g, '=== ')
      .replace(/function\s+(\w+)\s*\(/g, 'const _$1 = (');
    if (refactored !== code) {
  ...
  explainCode() {
    ...
    if (code.includes('function')) {
      addOutput('This code defines a function', 'log');
    }
    if (code.includes('for') || code.includes('while')) {
      addOutput('Contains loop structures for iteration', 'log');
    }
    if (code.includes('if')) {
      addOutput('Uses conditional statements for logic flow', 'log');
    }
    if (code.includes('class')) {
      addOutput('Defines a class with object-oriented structure', 'log');
    }
  }
  ...
  optimizeCode() {
    ...
    // Don't actually apply aggressive optimization
  ...
  const getConditionalValue0plr = (condition) => {
    if (condition) {
      return .replace('.js', '.ts') || 'converted.ts';
    createFile(filename, typescript);
    addOutput('🔷 Converted to TypeScript', 'success');
  }

  // AI API calls (when available)
  ...
  return getConditionalValue0plr();
})():', 'info');
  ...
  async callAI(prompt) {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        maxTokens: UI_CONSTANTS.ANIMATION_SLOW
      })
    });
